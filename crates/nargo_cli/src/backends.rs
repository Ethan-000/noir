cfg_if::cfg_if! {
    if #[cfg(feature = "axiom_halo2_backend")] {
        pub(crate) use halo2_backend::Halo2 as ConcreteBackend;
    } else {
        pub(crate) use acvm_backend_barretenberg::Barretenberg as ConcreteBackend;
    }
}

#[cfg(not(any(feature = "plonk_bn254", feature = "plonk_bn254_wasm", feature = "halo2_backend")))]
compile_error!("please specify a backend to compile with");

#[cfg(all(feature = "plonk_bn254", feature = "plonk_bn254_wasm", feature = "halo2_backend"))]
compile_error!(
